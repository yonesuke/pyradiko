...
class RadikoLoginAuth(contextlib.ContextDecorator):
    ...
    def login(self) -> None:
        ...
        login_json = requests.post(
            URL_LOGIN, data={"mail": self.mail, "pass": self.password}, timeout=TIMEOUT
        ).json()
    ...
    def auth2(self):
        ...
        partialkey = base64.b64encode(
            AUTHKEY_VAL[
                int(self.keyoffset) : int(self.keyoffset) + int(self.keylength)
            ].encode()
        )
...
class RadikoRecorder:
    ...
    def _get_playlist_url(self, station_id: str, is_areafree: bool) -> str:
        ...
        Returns:
            str: playlist URL
        """
        res = requests.get(f"{URL_STREAM_BASE}/{station_id}.xml", timeout=TIMEOUT)
        root = ET.fromstring(res.content)
        areafree_val = "1" if is_areafree else "0"

        for url_elem in root.findall(".//url"):
            if (
                url_elem.get("timefree") == "1"
                and url_elem.get("areafree") == areafree_val
            ):
        ...
        with self.radiko_util as radiko_util:
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                while left_sec > 0:
                    ...
                    chunk_len = CHUNK_MAX_SEC
                    if left_sec < CHUNK_MAX_SEC:
                        chunk_len = (
                            left_sec if left_sec % 5 == 0 else ((left_sec // 5) + 1) * 5
                        )

                    seek = seek_timestamp.strftime("%Y%m%d%H%M%S")
                    end_at = (
                        seek_timestamp + datetime.timedelta(seconds=chunk_len)
                    ...
                    url = (
                        ...
                        f"&l={chunk_len}&lsid={lsid}&type=c"
                    )
                    ...
                    chunk_sec = self._get_chunk_duration(str(chunk_file)) or chunk_len
                    left_sec -= chunk_sec
